...
int main() {
    ...
    int addrlen_a = sizeof(client_address_a);
    int addrlen_b = sizeof(client_address_b);
    int choice_a, choice_b;
    char judgment[MAX_BUFFER_SIZE] = {0};

    if ((server_socket_a = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
        die("Socket creation for clientA failed", false, false, 0, 0);
    }

    if ((server_socket_b = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
    ...
    if (listen(server_socket_a, 1) < 0 || listen(server_socket_b, 1) < 0) {
        die("Listen failed", true, true, server_socket_a, server_socket_b);
    }

    printf("Server is waiting for clients...\n");

    if ((client_socket_a = accept(server_socket_a, (struct sockaddr *)&client_address_a, (socklen_t *)&addrlen_a)) < 0) {
        die("Accept for clientA failed", true, true, server_socket_a, server_socket_b);
    }

    if ((client_socket_b = accept(server_socket_b, (struct sockaddr *)&client_address_b, (socklen_t *)&addrlen_b)) < 0) {
        die("Accept for clientB failed", true, true, server_socket_a, server_socket_b);
    }

    int play_again = 1;

    while (play_again) {
        valread = recv(client_socket_a, &choice_a, sizeof(choice_a), 0);
        if (valread < 1) break;

        valread = recv(client_socket_b, &choice_b, sizeof(choice_b), 0);
        if (valread < 1) break;
        ...
        send(client_socket_a, judgment, strlen(judgment), 0);
        send(client_socket_b, judgment, strlen(judgment), 0);
        ...
        recv(client_socket_a, play_response_a, MAX_BUFFER_SIZE, 0);
        recv(client_socket_b, play_response_b, MAX_BUFFER_SIZE, 0);

        if (strcmp(play_response_a, "yes") != 0 || strcmp(play_response_b, "yes") != 0) {
            play_again = 0;
        }

        send(client_socket_a, &play_again, sizeof(play_again), 0);
        send(client_socket_b, &play_again, sizeof(play_again), 0);
    }
