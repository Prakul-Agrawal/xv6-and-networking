...
void* ack_thread(void* sockfdptr) {
    ...
    while (1) {
        ...
        recvfrom(sockfd, buf, sizeof(buf), 0, (struct sockaddr*)&client_addr, &addr_len);
...
void* resend_thread(void* sockfdptr) {
    ...
    while (1) {
        ...
        if (!flag) {
            ...
            sendto(sockfd, "T", 1, 0, (struct sockaddr*)&client_addr, sizeof(client_addr));
            return NULL;
        }
    }
}
...
void receive_data(int sockfd) {
    ...
    recvfrom(sockfd, num_chunks_str, sizeof(num_chunks_str), 0, (struct sockaddr*)&client_addr, &addr_len);
    ...
    while (1) {
        ...
        int valread = recvfrom(sockfd, chunk, sizeof(chunk), 0, (struct sockaddr*)&client_addr, &addr_len);
        if (valread < 1) {
            perror("recvfrom");
            close(sockfd);
            exit(EXIT_FAILURE);
...
int main() {
    ...
    while (1) {
        ...
        sendto(sockfd, num_chunks_str, sizeof(num_chunks_str), 0, (struct sockaddr*)&client_addr, sizeof(client_addr));
        ...
        for (int i = 0; i < num_chunks; i++) {
            ...
            sendto(sockfd, chunk, strlen(chunk), 0, (struct sockaddr*)&client_addr, sizeof(client_addr));
            printf("Sent chunk %d\n", i);
            printf("Chunk %d: %s\n", i, chunk);
        }
